...
function UpcomingExpoCard({ expo }: { expo: { id: string; expoName: string; expoDate: string; venue: string; status: string } }) {
  return (
    <Link href={`/visitor/expos/${expo.id}`} className="block rounded-2xl border border-border/70 bg-elevated p-4 transition hover:border-primary/25 hover:bg-elevated/80 focus:outline-none focus:ring-4 focus:ring-primary/10">
          ...
          <Card className="p-5 sm:p-6">
              ...
              <div className="space-y-3">
                 {recentActivity.slice(0, 5).map((activity) => (
                   <div key={activity.id} className="flex items-center gap-3 rounded-2xl border border-border/60 bg-elevated p-3">
